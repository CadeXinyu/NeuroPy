...
def hilbert_amplitude_stat(signals: np.array, freq_band, fs, statistic="mean"):
...
def hilbert_amplitude_stat(signal: Signal, freq_band, statistic="mean", verbose=False):
    """
    Calculate Hilbert amplitude statistic over the entire signal.
    
    Applies bandpass filtering, computes Hilbert transform envelope,
    and calculates specified statistic for each channel.
    
    Parameters
    ----------
    signal : Signal
        Input Signal object containing multi-channel data
    freq_band : tuple
        Frequency band for filtering (low_freq, high_freq) in Hz
    statistic : str, optional
        Statistical measure to compute: "mean", "median", or "std"
        Default is "mean"
    verbose : bool, optional
        Whether to print MNE filtering progress. Default is False
    
    Returns
    -------
    np.ndarray
        Array of shape (n_channels,) containing the computed statistic
        for each channel's Hilbert envelope
    
    Examples
    --------
    >>> bandpower = hilbert_amplitude_stat(signal, (8, 12), statistic="mean")  # Alpha band
    """
    
    # Define statistic function using dictionary lookup (more efficient than if-elif chain)
    stat_functions = {
        "mean": np.mean,
        "median": np.median, 
        "std": np.std
    ...
    if statistic not in stat_functions:
        raise ValueError(f"statistic must be one of {list(stat_functions.keys())}, got '{statistic}'")
    
    get_stat = stat_functions[statistic]
    
    # Create a copy to avoid modifying original signal
    # Apply bandpass filter and Hilbert transform to get amplitude envelope
    signal_filtered = signal.copy()
    signal_filtered.mne.filter(
        l_freq=freq_band[0], 
        h_freq=freq_band[1], 
        verbose=verbose
    )
    signal_filtered.mne.apply_hilbert(envelope=True)
    
    # Sync the traces with MNE object after filtering and Hilbert transform
    signal_filtered.update()
    
    # Get filtered and enveloped traces
    traces = signal_filtered.traces
    
    # Compute statistic for each channel using vectorized operation
    # More efficient than loop for large arrays
    bandamp_stat = np.apply_along_axis(get_stat, axis=1, arr=traces)
